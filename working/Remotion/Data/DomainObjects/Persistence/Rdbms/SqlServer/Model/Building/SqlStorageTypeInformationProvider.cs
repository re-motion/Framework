  ...
  public class SqlStorageTypeInformationProvider : IStorageTypeInformationProvider
  {
    ...
    public IStorageTypeInformation GetStorageType (object value)
    {
      if (value == null)
      {
        ...
        return new StorageTypeInformation (typeof (object), "nvarchar (max)", DbType.String, true, 4000, typeof (object), NullValueConverter.Instance);
      }
    ...
    private IStorageTypeInformation GetStorageType (Type dotNetType, int? maxLength, bool isNullableInDatabase)
    {
      ...
      if (ReflectionUtility.IsStringPropertyValueType (dotNetType))
      {
        ...
        var storageTypeInformation = new StorageTypeInformation (typeof (string), storageTypeName, DbType.String, isNullableInDatabase, maxLength ?? StorageTypeLengthRepresentingMax, dotNetType, new DefaultConverter (dotNetType));

        if (storageTypeInformation.StorageTypeLength == StorageTypeLengthRepresentingMax)
