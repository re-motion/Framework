...
namespace Remotion.Collections
{
  ...
  public sealed class ConcurrentDataStore<TKey, TValue> : IDataStore<TKey, TValue>, IEnumerable<KeyValuePair<TKey, TValue>>
  {
    ...
    private class Boxed
    {
      public static readonly Boxed ExceptionSentinel = new Boxed (default (TValue));
    ...
    public bool Remove (TKey key)
    {
      ...
      if (TryGetValueInternal (key, out value))
      {
        ...
        return _innerDictionary.TryRemove (key, out synchronizedValue);
      }
    ...
    public TValue GetOrCreateValue (TKey key, Func<TKey, TValue> valueFactory)
    {
      ...
      {
        TValue resultValue;
        if (TryGetValueInternal (key, out resultValue))
          return resultValue;
    ...
    {
      SynchronizedValue synchronizedValue;
      if (_innerDictionary.TryGetValue (key, out synchronizedValue))
      {
        if (synchronizedValue.Boxed == null)
        {
          // Double-checked lock
