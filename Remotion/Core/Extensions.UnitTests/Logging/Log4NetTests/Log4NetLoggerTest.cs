...
public class Log4NetLoggerTest
{
  ...
  {
    Assert.That(Log4NetLogger.Convert(MicrosoftLogging.LogLevel.Debug), Is.EqualTo(Level.Debug));
    Assert.That(Log4NetLogger.Convert(MicrosoftLogging.LogLevel.Information), Is.EqualTo(Level.Info));
    Assert.That(Log4NetLogger.Convert(MicrosoftLogging.LogLevel.Warning), Is.EqualTo(Level.Warn));
    Assert.That(Log4NetLogger.Convert(MicrosoftLogging.LogLevel.Error), Is.EqualTo(Level.Error));
    Assert.That(Log4NetLogger.Convert(MicrosoftLogging.LogLevel.Critical), Is.EqualTo(Level.Critical));
  }

  [Test]
  public void Convert_WithInvalidLogLevel_ThrowsArgumentException ()
  {
    Assert.That(() => Log4NetLogger.Convert(MicrosoftLogging.LogLevel.None),
        Throws.TypeOf<ArgumentException>().With.ArgumentExceptionMessageEqualTo("LogLevel does not support value None.", "logLevel"));
  }
  ...
  public void Log_WritesCorrectlyFormattedMessage (string level, MicrosoftLogging.LogLevel logLevel)
  {
    ...
    InitializeConsole(MicrosoftLogging.LogLevel.Debug);
    logger.Log(logLevel, new MicrosoftLogging.EventId(1, "event"), "state", null, (_, _) => "LoggingTest");

    CheckConsoleOutput($"{level}: LoggingTest");
  }
