...
namespace Remotion.Data.DomainObjects.UnitTests.IntegrationTests.Transaction
{
  ...
  public class TryEnsureDataAvailableTest : EnsureDataAvailableTestBase
  {
    ...
    public void TryEnsureDataAvailable_NotLoadedYet ()
    {
      ...
      ListenerMock.Verify(
          mock => mock.ObjectsLoading(TestableClientTransaction, It.Is<ReadOnlyCollection<ObjectID>>(_ => new[] { NotLoadedObject1.ID }.All(_.Contains))),
          Times.AtLeastOnce());
      ListenerMock.Verify(
          mock => mock.ObjectsLoaded(TestableClientTransaction, It.Is<ReadOnlyCollection<IDomainObject>>(_ => new[] { NotLoadedObject1 }.All(_.Contains))),
          Times.AtLeastOnce());
    ...
    public void TryEnsureDataAvailable_Many_SomeLoadedSomeNot ()
    {
      ...
      ListenerMock.Verify(
          ...
          Times.AtLeastOnce());
      ListenerMock.Verify(
          mock => mock.ObjectsLoaded(TestableClientTransaction, It.Is<ReadOnlyCollection<IDomainObject>>(p => p.SetEquals(new[] { NotLoadedObject1, NotLoadedObject2 }))),
          Times.AtLeastOnce());

      ListenerMock.Verify(
          mock => mock.ObjectsLoading(It.IsAny<ClientTransaction>(), It.Is<ReadOnlyCollection<ObjectID>>(_ => new[] { LoadedObject1.ID }.All(_.Contains))),
          Times.Never());
