...
{
  /// <summary>
  /// Abstract base class for <see cref="IQueryFileFinder"/> implementations that want to search for a single required query file.
  /// </summary>
  public abstract class BaseDirectoryBasedRequiredQueryFileFinder : IQueryFileFinder
  {
    private readonly string _queryFile;

    protected BaseDirectoryBasedRequiredQueryFileFinder (IAppContextProvider appContextProvider, string queryFile)
    {
    ...
    public IEnumerable<string> GetQueryFilePaths ()
    {
      return File.Exists(_queryFile)
          ...
          : throw new ConfigurationException($"The query file '{_queryFile}' does not exist.");
    }
  }
}
