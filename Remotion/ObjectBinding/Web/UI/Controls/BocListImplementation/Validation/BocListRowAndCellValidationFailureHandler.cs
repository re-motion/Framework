...
{
  /// <summary>
  /// Handles all still unhandled validation failures and should always be the last handler to run.
  /// </summary>
  ...
  public class BocListRowAndCellValidationFailureHandler : IBocListValidationFailureHandler
  {
    ...
    public void HandleValidationFailures (ValidationFailureHandlingContext context)
    {
      ...
      var hasRowsWithUnhandledValidationFailures = context.ValidationFailureRepository.GetUnhandledValidationFailuresForBocListAndContainingDataRowsAndDataCells(true).Any();
