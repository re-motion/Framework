...
namespace Remotion.ObjectBinding.Web.Validation.UnitTests.Factories.Filtering
{
  ...
  {
    [Test]
    [TestCase (true, true, new[] { typeof (BusinessObjectBoundEditableWebControlValidationResultDispatchingValidator), typeof (RequiredFieldValidator) })]
    ...
    public void CreateValidators (bool required, bool isValueType, Type[] expectedValidatorTypes)
    {
      var compoundFactory =
          new CompoundBocTextValueValidatorFactory (
              ...
              { new BocTextValueValidatorFactory(), new ValidationBusinessObjectBoundEditableWebControlValidatorFactory() });
