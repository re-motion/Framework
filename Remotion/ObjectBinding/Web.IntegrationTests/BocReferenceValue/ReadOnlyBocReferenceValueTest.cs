...
{
  [TestFixture]
  public class ReadOnlyBocReferenceValueTest : IntegrationTest
  {
    [Test]
    public void Validate_OnReadOnlyControl_CreatesValidationErrors ()
    {
      ...
      var readOnlyControl = home.ReferenceValues().GetByLocalID("PartnerField_ReadOnly");
      var validationErrors = readOnlyControl.GetValidationErrorsForReadOnly();

      Assert.That(validationErrors, Is.EqualTo(new[] { "Localized invalid partner." }));

      readOnlyControl = home.ReferenceValues().GetByLocalID("PartnerField_ReadOnlyWithoutSelectedValue");
