...
{
  /// <summary>
  /// Defines a rule associated with an <see cref="ITypeInformation"/> which specifies the validators to remove from the validation specification. The rule belongs to a component 
  ...
  /// </summary>
  /// <seealso cref="RemovingObjectValidationRuleCollector"/>
  public interface IRemovingObjectValidationRuleCollector
  {
    /// <summary>
    ...
    /// <summary>
    /// Gets the <see cref="Type"/> for which the validators should be removed.
    /// </summary>
    ...
    /// </summary>
    IEnumerable<RemovingObjectValidatorRegistration> Validators { get; }

    /// <summary>
    /// Specifies that all validators of <paramref name="validatorType"/> should be removed.
    /// Note: It is only supported to remove validators which are registered with the <see cref="IAddingObjectValidationRuleCollector.IsRemovable"/> flag set to <see langword="true" />.
