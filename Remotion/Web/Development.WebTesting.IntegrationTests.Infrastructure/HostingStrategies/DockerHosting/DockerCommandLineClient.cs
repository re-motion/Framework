...
namespace Remotion.Web.Development.WebTesting.IntegrationTests.Infrastructure.HostingStrategies.DockerHosting
{
  ...
  public class DockerCommandLineClient : IDockerClient
  {
    ...
    public void Pull (string imageName)
    {
      ...
      RunDockerCommand (string.Format ("pull {0}", imageName), timeout: _pullTimeout);
    }
        ...
        string args)
    {
      ...
      if (ports.Any())
      {
        var portFlags = string.Join (" ", ports.Select (kvp => string.Format ("-p {0}:{1}", kvp.Key, kvp.Value)));
        commandBuilder.Append (portFlags).Append (' ');
      }

      if (mounts.Any())
      {
        var mountFlags = string.Join (" ", mounts.Select (kvp => string.Format (@"-v ""{0}"":""{1}""", kvp.Key, kvp.Value)));
        commandBuilder.Append (mountFlags).Append (' ');
      }

      if (entryPoint != null)
        commandBuilder.Append (string.Format (@"--entrypoint=""{0}""", entryPoint)).Append (' ');

      if (hostname != null)
        commandBuilder.Append (string.Format (@"--hostname ""{0}""", hostname)).Append (' ');
    ...
    public bool ContainerExists (string containerName)
    {
      ...
      using (var p = Process.Start (_dockerExeFullPath, string.Format ("inspect {0}", containerName)))
      {
    ...
    {
      s_log.Info (string.Format ("Running: 'docker {0}'", dockerCommand));
      ...
      using (var dockerProcess = new Process { StartInfo = startInfo })
      {
        ...
        {
          var errorMessage = string.Format ("Docker command '{0}' failed: {1}", dockerCommand, error);
    ...
    private void WaitForExit (Process dockerProcess, string dockerCommand, TimeSpan timeout)
    {
      ...
      while (!dockerProcess.HasExited)
      {
        ...
        if (stopwatch.ElapsedMilliseconds > timeout.TotalMilliseconds)
          throw new TimeoutException (string.Format ("Docker command '{0}' ran longer than the configured timeout of '{1}'.", dockerCommand, timeout));
      }
    }
