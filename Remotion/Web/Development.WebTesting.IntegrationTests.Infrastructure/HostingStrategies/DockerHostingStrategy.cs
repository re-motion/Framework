...
namespace Remotion.Web.Development.WebTesting.IntegrationTests.Infrastructure.HostingStrategies
{
  ...
  public class DockerHostingStrategy : IHostingStrategy
  {
        ...
        [NotNull] string mounts)
    {
      ...
      _webApplicationRoot = new Uri (string.Format ("http://{0}:{1}/", hostname ?? "localhost", port));
    ...
    private void VerifyWebApplicationStarted (Uri webApplicationRoot, TimeSpan applicationPingTimeout)
    {
      ...
      s_log.Info (string.Format ("Verifying that '{0}' is accessible within {1}.", resolvedUri, applicationPingTimeout));
      ...
      while (statusCode != HttpStatusCode.OK)
      {
        ...
        {
          CheckTimeout (webApplicationRoot, applicationPingTimeout, stopwatch, string.Format ("Failed with WebException '{0}'", ex.Message));
        }

        CheckTimeout (webApplicationRoot, applicationPingTimeout, stopwatch, string.Format ("Failed with HttpStatusCode '{0}'", statusCode));
      ...
      s_log.Info (string.Format ("Verified that '{0}' is accessible after {1:N0} ms.", resolvedUri, stopwatch.Elapsed.TotalMilliseconds));
    }
    ...
    private void CheckTimeout (Uri webApplicationRoot, TimeSpan applicationPingTimeout, Stopwatch stopwatch, string failureReason)
    {
      if (stopwatch.ElapsedMilliseconds > applicationPingTimeout.TotalMilliseconds)
      {
        throw new WebException (
            string.Format (
                "Checking the web application root '{0}' did not return '{1}' in the defined {2} ({3}). ",
                webApplicationRoot,
                HttpStatusCode.OK,
                "applicationPingTimeout",
                applicationPingTimeout)
            + string.Format ("{0}.", failureReason),
            WebExceptionStatus.Timeout);
      }

      s_log.Warn (
          string.Format (
              "Checking the web application root '{0}' failed with following reason: {1}. Retrying until {2} ({3}) is reached.",
              webApplicationRoot,
              failureReason,
              "applicationPingTimeout",
              applicationPingTimeout));
