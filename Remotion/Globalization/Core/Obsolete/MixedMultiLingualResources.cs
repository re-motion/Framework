...
namespace Remotion.Mixins.Globalization
{
  ...
  public static class MixedMultiLingualResources
  {
    ...
    [Obsolete(
        "Retrieve IGlobalizationService from IoC container and use IGlobalizationService.GetResourceManager (objectType). "
        + "Note: When using IGlobalizationService, the order of resolution has changed to return resources for mixins first, then the target types. (Version 1.13.223.0)",
        true)]
    ...
    [Obsolete(
        "Retrieve IGlobalizationService from IoC container and use IGlobalizationService.GetResourceManager (objectTypeToGetResourceFor).GetString (name). "
        + "Note: This method did not include the hierarchy but IGlobalizationService will always include the hierarchy. "
        + "Note: When using IGlobalizationService, the order of resolution has changed to return resources for mixins first, then the target types. (Version 1.13.223.0)",
        true)]
    ...
    [Obsolete(
        "Retrieve IGlobalizationService from IoC container and test for IGlobalizationService.GetResourceManager (objectTypeToGetResourceFor).ContainsString (name). "
        + "Note: This method did not include the hierarchy but IGlobalizationService will always include the hierarchy. (Version 1.13.223.0)",
        true)]
