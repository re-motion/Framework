...
using static Customizations.SqlServers;
...
class Build : RemotionBuild
{
  ...
  public override void ConfigureProjects (ProjectsBuilder projects)
  {
    ...
    projects.AddReleaseProject("Core.Core")
        .SetCreateNugetPackageWithSymbolServerSupport(true);
    projects.AddReleaseProject("Core.Documentation")
        .SetCreateDocumentationFile(true);
  ...
  public override void ConfigureSupportedTestDimensions (SupportedTestDimensionsBuilder supportedTestDimensions)
  {
    ...
    supportedTestDimensions.AddSupportedDimension<Browsers>(
        NoBrowser,
        Chrome, Firefox, Edge);
    supportedTestDimensions.AddSupportedDimension<SqlServers>(
  ...
  public override void ConfigureEnabledTestDimensions (EnabledTestDimensionsBuilder enabledTestDimensions)
  {
    ...
    if (SupportedTestDimensions.IsSupported<SqlServers>())
    {
      var testSqlServers = SupportedTestDimensions.ParseTestDimensionValuesOrDefault<SqlServers>(TestSqlServers)
                           ?? throw CreateConfigurationException<SqlServers>();
  ...
  public override void ConfigureTestMatrix (TestMatricesBuilder builder)
  {
    builder.AddTestMatrix(
        ...
        new TestDimension[,] // todo docker images need to be wired to the config file
        {
            ...
            { Chrome, NET481, Release, x86, NoDB, EnforcedLocalMachine },
            { Firefox, NET48, Release, x64, NoDB, EnforcedLocalMachine },
            { Edge, NET48, Release, x64, NoDB, EnforcedLocalMachine },
  ...
  protected IEnumerable<string> SupportedTestSqlServers => GetTestDimensionValueList<SqlServers>();
}
